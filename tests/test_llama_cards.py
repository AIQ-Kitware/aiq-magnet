...
def override_path(card, corrected_path):
    ...
    if card.has_pipeline:
        card.pipeline['llama_predict']['algo_params']['helm_runs_path'] = corrected_path
    else:
...
def run_download(tmp_path_factory):
    ...
    download_helm_results.main(argv=False, download_dir=helm_dir, benchmark='lite', version='v1.0.0', runs='regex:mmlu.*model=.*llama.*',)
    download_helm_results.main(argv=False, download_dir=helm_dir, benchmark='lite', version='v1.2.0', runs='regex:mmlu.*model=.*llama.*',)
