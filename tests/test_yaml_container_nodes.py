...
class Infer(MagnetYamlProcessNode):
...
class LeaseOnlyInfer(LeasedYamlProcessNode):
    """Lease capability without the container capability."""
...
def test_execution_capabilities_are_independent_and_composable():
    assert issubclass(ContainerYamlProcessNode, YamlProcessNode)
    assert issubclass(LeasedYamlProcessNode, YamlProcessNode)
    assert issubclass(MagnetYamlProcessNode, YamlProcessNode)

    container = ContainerYamlProcessNode()
    leased = LeasedYamlProcessNode()
    combined = MagnetYamlProcessNode()
    assert containers.is_container_capable(container)
    assert not leasing.is_lease_capable(container)
    assert leasing.is_lease_capable(leased)
    assert not containers.is_container_capable(leased)
    assert containers.is_container_capable(combined)
    assert leasing.is_lease_capable(combined)
...
def test_a_leased_yaml_node_does_not_become_containerized():
    spec = {'nodes': {'work': {
        'class': f'{__name__}.LeaseOnlyInfer',
        'executable': 'python -m pkg.infer',
        'algo_params': {'model_id': None},
        'out_paths': {'results_fpath': 'results.json'},
    }}}
    node = _node(
        spec, {'model_id': 'qwen3-8b'}, settings=_settings(),
        lease=leasing.LeaseSettings(enabled=True),
    )
    assert 'infer-stack run' in node.command
    assert 'docker run' not in node.command
...
def test_a_card_can_declare_endpoint_params():
    ...
    closed node-spec allow-list for the leasing capability's four keys.
...
def test_an_image_that_reaches_no_node_is_an_error():
    ...
    assert 'MagnetYamlProcessNode' in message
