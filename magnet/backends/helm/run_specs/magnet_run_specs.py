...
def get_local_dataset_meta_spec(original_spec: str, instances_path: str, **kwargs) -> RunSpec:
    original_run_spec_function = get_run_spec_function(original_spec)
    if original_run_spec_function is None:
        raise ValueError(f'Unknown HELM run spec: {original_spec!r}')

    original_run_spec = original_run_spec_function(**kwargs)
