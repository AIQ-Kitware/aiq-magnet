...
class GSCLIBackend(_GSBaseBackend):
    ...
    # Transfers
    def download_version(self, bench: str, version: str, dest: ub.Path, checksum: bool = False) -> None:
        bucket_base = f"{self.bucket}/{bench}/benchmark_output/runs"
        version_src_gs = f"{bucket_base}/{version}"
        dest = ub.Path(dest).ensuredir()
        self._gsutil_rsync(version_src_gs, str(dest), checksum=checksum)

    def download_runs(self, bench: str, version: str, dest: ub.Path, run_ids: List[str], checksum: bool = False) -> None:
        bucket_base = f"{self.bucket}/{bench}/benchmark_output/runs"
        version_src_gs = f"{bucket_base}/{version}"
        dest = ub.Path(dest).ensuredir()
        for r in run_ids:
...
class GSFSSspecBackend(_GSBaseBackend):
    ...
    def download_version(self, bench: str, version: str, dest: ub.Path, checksum: bool = False) -> None:
        ...
        version_src_gs = f"{bucket_base}/{version}"
        if checksum:
            eprint("Note: checksum is not supported with backend=gcsfs; proceeding without.")
...
def _do_requested_download(backend, benchmark, version, dest, verbose, runs, checksum):
    ...
    try:
        if runs:
            ...
            backend.download_runs(benchmark, version, dest, matched, checksum=bool(checksum))
        else:
...
def main(argv=None, **kwargs) -> int:
    ...
    if version == "auto":
        eprint(f"Resolving latest version for benchmark '{args.benchmark}' (backend={args.backend})...")
    ...
    bucket_base = f"{backend.bucket}/{benchmark}/benchmark_output/runs"
    src = f"{bucket_base}/{version}"
    dest_root = ub.Path(args.download_dir).expanduser().resolve() / benchmark / "benchmark_output" / "runs"
    ...
    # Idempotent sync
    ret = _do_requested_download(backend, benchmark, version, dest, verbose, runs, checksum)
    return ret
